...
static void multi_protocol_spp_receive_callback(void *priv, u8 *buf, u16 len)
{
     JL_protocol_data_recieve(priv, buf, len);
    log_info("-----------samson---len=%d----data recieve:\n",len);
...
void multi_protocol_common_callback_init(void)
{
    ...
    app_spp_recieve_callback_register(multi_protocol_spp_hdl, multi_protocol_spp_receive_callback); 
    //app_spp_state_callback_register(multi_protocol_spp_hdl, spp_state_cbk);
    //app_spp_wakeup_callback_register(multi_protocol_spp_hdl, app_spp_data_send);
