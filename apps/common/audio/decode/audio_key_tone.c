...
static const char *const key_tone_index[] = {
    ...
    [KTONE_IDEX_NUM_2] 		= SDFILE_RES_ROOT_PATH"tone/key.*",
};
...
{
    if ((get_call_status() == BT_CALL_OUTGOING) || (get_call_status() == BT_CALL_INCOMING)|| (get_call_status() == BT_CALL_ACTIVE)) {
        return;
    }
    audio_key_tone_play_index(KTONE_IDEX_NUM_2, 0);
