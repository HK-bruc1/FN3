...
#define CHARGING_POWER_PERCENT_UPDATE_TIME  (16*1000) //充电时，电量更新周期
#define NORMAL_POWER_PERCENT_UPDATE_TIME    (8*1000) //非充电时，电量更新周期

static u8 percent_save = 0xff;
static u8 local_detail_percent = 0;
static u8 remote_detail_percent = 0;
static u16 remote_vbat = 0;
static u8 is_power_up;
...
__attribute__((weak)) u8 remap_calculate_vbat_percent(u16 bat_val)
{
    ...
    if(temp_percent == 0xff 
        || (is_power_up && sys_timer_get_ms() < 2000 && get_charge_online_flag() == 0 
    ...
    if(get_charge_online_flag()){ //充电中
        ...
        if(percent == 100 && temp_percent > 90 && bat_val >= 414){
            // temp_percent = 100;//避免跳100慢     //GZH 91%直接跳100% ，仓显示99% 问题
        }
    }
    else{ //非充电中
        if(sys_timer_get_ms() >= time_ms + NORMAL_POWER_PERCENT_UPDATE_TIME){
            time_ms = sys_timer_get_ms();
            u8 change = 0;
            if(temp_percent > percent && temp_percent > local_detail_percent){
                temp_percent--; //只允许变小
            }
            // G_log(" 放电电量  ");
            printf("bat_val %d --> percent %d,    temp_percent %d    %d %d\n", bat_val, percent, temp_percent, percent_save, local_detail_percent);
                        // 402       百分比 80    保存显示的电量 82   83 88
