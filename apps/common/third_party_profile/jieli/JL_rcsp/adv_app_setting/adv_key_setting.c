...
void set_key_setting(u8 *key_setting_info)
{
    ...
    _s_info.key_setting[3 * 4 + 2] = key_setting_info[2];
    _s_info.key_setting[3 * 6 + 2] = key_setting_info[3];

    _s_info.key_setting[3 * 1 + 2] = key_setting_info[4];
    _s_info.key_setting[3 * 3 + 2] = key_setting_info[5];
    _s_info.key_setting[3 * 5 + 2] = key_setting_info[6];
    _s_info.key_setting[3 * 7 + 2] = key_setting_info[7];
...
void get_key_setting(u8 *key_setting_info)
{
    ...
    key_setting_info[2] = _s_info.key_setting[3 * 4 + 2];
    key_setting_info[3] = _s_info.key_setting[3 * 6 + 2];

    key_setting_info[4] = _s_info.key_setting[3 * 1 + 2];
    key_setting_info[5] = _s_info.key_setting[3 * 3 + 2];
    key_setting_info[6] = _s_info.key_setting[3 * 5 + 2];
    key_setting_info[7] = _s_info.key_setting[3 * 7 + 2];
}

static void update_key_setting_vm_value(u8 *key_setting_info)
{
    syscfg_write(CFG_RCSP_ADV_KEY_SETTING, key_setting_info, ADVKEYMAX);
}
...
{
    u8 key_setting[ADVKEYMAX];
    if (!key_setting_info) {
        get_key_setting(key_setting);
    } else {
        memcpy(key_setting, key_setting_info, ADVKEYMAX);
    }
    // 1、写入VM
    if (write_vm) {
