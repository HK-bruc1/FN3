...
struct t_s_info _s_info = {
   ...
   /*1col 01:左耳 02 右耳；2col key 1:单击 2双击 3 长按 4三击；3col event;*/
    ...
    .mic_mode = 1,
    .work_mode = 0, //音乐模式为1，游戏模式为2，改0避免开机后app下发默认数据时报音乐模式提示音
    .eq_mode_info = {0x00},
};
    ...
    case ATTR_TYPE_KEY_SETTING:
        ...
        while (dlen >= 3) {
            if (pbuf[0] == 0x01) {//左耳
                ...
                } else if (pbuf[1] == 0x02) {//双击
                    _s_info.key_setting[8] = pbuf[2];
                }else if (pbuf[1] == 0x03) {//长按
                    _s_info.key_setting[14] = pbuf[2];
                } else if (pbuf[1] == 0x04) {//三击
                    _s_info.key_setting[20] = pbuf[2];
                }
            } else if (pbuf[0] == 0x02) { //右耳
                if (pbuf[1] == 0x01) {
...
static void earphone_mute_handler(u8 *other_opt, u32 msec)
{
    ...
    // G_log("-------------------------------------  tmp_param  len = %d    ",sizeof(tmp_param));
    // put_buf(tmp_param,sizeof(tmp_param));

#if TCFG_USER_TWS_ENABLE

    //tmp_param[0] 通道  0：左右   1：L   2：R
    //tmp_param[1] 使能位
...
//samson add by on 20250328
int rcsp_data_send(void *priv, u8 *buf, u16 len)
{

   if (JL_bt_chl==RCSP_BLE)
   {
      return rcsp_ble_data_send(priv,buf,len);
   }
   else
   {
      return rcsp_spp_data_send(priv,buf,len);
   }
...
static void rcsp_spp_protocol_data_recieve(void *priv, void *buf, u16 len)
{
    ...
    rcsp_printf("------rcsp_spp_protocol_data_recieve-----samson---len=%d----data recieve:\n",len);
    rcsp_printf_buf(buf,len);
    //os_time_dly(100);
    DHFAPCommand_DataPut(buf,len);
}
...
#endif
  //  if(JL_bt_chl == type)//added by samson
   //   return ;
    if (type == RCSP_BLE) {
#if 1//RCSP_UPDATE_EN
        JL_bt_chl = RCSP_BLE;
#endif
         DHFCommon_SetRCSPType(0);//added by samson
        ...
        rcsp_ble_callback_set(JL_protocol_resume, rcsp_spp_protocol_data_recieve, JL_ble_status_callback);//samson modify by 20250409
